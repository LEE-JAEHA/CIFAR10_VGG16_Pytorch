...
class BasicBlock(nn.Module):
    ...
    def __init__(self,in_planes,out_planes,stride=1):
        ...
        if stride!=1 or in_planes != self.expansion * out_planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*out_planes, kernel_size=3, stride=stride, padding=1, bias=False),
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        strides = [stride] + [1] * (num_blocks - 1) # 각 layer 개수 만큼 stride 1 값을 넣어 주기 위해
        layers = []
